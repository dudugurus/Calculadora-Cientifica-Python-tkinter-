# CALCULADORA Cientifica 2.34 aceita expressoes,TKINTER (18/08/2022)  (12/11/2022)
...
janela.title("Calculadora Cientifica V2.34 By Edu (12/11/2022)") 
...
saida=Entry(janela,text=inp,width=25,justify='right',bg="light green",font=('arial',10),bd=4) 
saida.grid(row=1,column=1, columnspan=4, padx=5,pady=15, ipady=5) 
saida.config(font=("arial",11,"bold"))
...
def f2_0(op=""): # Zera 2ª função ou Ativa
    ...
    if f2==1: # se esta ativada desativa
        ...
        btn_dict["btn_x²"].configure(text=f"x²")
        ...
        btn_dict["btn_x²"].configure(text=f"fatX")
        btn_dict["btn_."].configure(text=f",")
        btn_dict["btn_("].configure(text=f"{p})")
...
def fatorando(n):
    ...
    while n >= impar :
        while not n%impar:
            fatores.append(impar)
            n = n//impar
        if n%impar>0 and n>= impar:
            if impar==2:
                impar=3
            else:
                impar=impar+2
            while  n%impar>0 and  n>= impar:
                impar += 2
    return fatores

def fatX(num):
    fatores =  fatorando(num)
    if len(fatores)==1:
        return str(num)
    else:
        tam=len(fatores)
        ...
        if tam>1:
            ...
            for i in range(1,tam):
                if pri==fatores[i] :
                    ...
                    if cont>1:
                        formula=formula+"**"+str(cont)
                    pri=fatores[i]
                    cont=1
                    formula=formula+")*("+str(pri)
            if cont>1 and pri==fatores[i]:
                formula=formula+"**"+str(cont)
        return f"{formula})"
...
def copiar():
    saida.select_range(0,'end')
    saida.event_generate("<<Copy>>")

def colar():
    saida.event_generate("<<Paste>>")
...
mm = Menu(saida)
mm.add_command(label="Copiar",command=copiar)
mm.add_separator()
mm.add_command(label="Colar",command=colar)
saida.bind("<Button-3>",menucop)
...
def Calcula(event):
    ...
    try: 
        if button == u"\u221Ax":
            ...
            if float(eval(inp.get()))<2: a=1/0
            last_ant.append(last)
            last = inp.get()
            resp = fatX(eval(inp.get()))
            inp.set(str(resp))
        elif button == "R2(":
            p = str(int("0"+p)+ 1)
            inp.set(inp.get()+"R2(")
